...
getgenv().write_end_server_value = function(value)
    local file_name = "should_end_server.txt"
    if value ~= "yes" and value ~= "no" then return false end
    if writefile and typeof(writefile) == "function" then writefile(file_name, value) end
    return true
end

getgenv().read_end_server_raw = function()
    local file_name = "should_end_server.txt"
    if not isfile(file_name) then writefile(file_name, "no") return "no" end
...
getgenv().should_end_server = function()
    ...
    if content == "yes" then return true end
    return false
end
wait(0.1)
...
if not getgenv().Already_Running_Flames_Teleport_Checker then
    ...
    LocalPlayer.OnTeleport:Connect(function(State)
        if (not TeleportCheck) and queue_tp then
            TeleportCheck = true
            queue_tp("loadstring(game:HttpGet('https://gitlab.com/greatest-group/experience_coding/-/raw/main/Extra/Condo_GUI.lua?ref_type=heads'))()")
        end
    end)
    if getgenv().notify then
...
if not low_level_executor() then
    task.spawn(function()
        ...
        if not getgenv().InitializedAdonisAdmin_Bypass then
            loadstring(game:HttpGet("https://pastefy.app/Ow6wwuK7/raw",true))()
            wait(0.1)
...
if not getgenv().PlayerGui then getgenv().PlayerGui = PlayerGui end
local Lighting = cloneref and cloneref(game:GetService("Lighting")) or game:GetService("Lighting")
if not getgenv().Lighting then getgenv().Lighting = Lighting end
local Workspace = cloneref and cloneref(game:GetService("Workspace")) or game:GetService("Workspace")
if not getgenv().Workspace then getgenv().Workspace = Workspace end
local ReplicatedStorage = cloneref and cloneref(game:GetService("ReplicatedStorage")) or game:GetService("ReplicatedStorage")
if not getgenv().ReplicatedStorage then getgenv().ReplicatedStorage = ReplicatedStorage end
local RunService = cloneref and cloneref(game:GetService("RunService")) or game:GetService("RunService")
if not getgenv().RunService then getgenv().RunService = RunService end
local Camera = Workspace.CurrentCamera
if not getgenv().CurrentCamera then getgenv().CurrentCamera = Camera end
local StarterGui = cloneref and cloneref(game:GetService("StarterGui")) or game:GetService("StarterGui")
if not getgenv().StarterGui then getgenv().StarterGui = StarterGui end
local UserInputService = cloneref and cloneref(game:GetService("UserInputService")) or game:GetService("UserInputService")
if not getgenv().UserInputService then getgenv().UserInputService = UserInputService end
...
if not getgenv().Backpack then getgenv().Backpack = Backpack end
local Rayfield = nil
...
if game.Players.LocalPlayer.UserId == 10483028410 then
    for _, v in ipairs(ReplicatedStorage:GetChildren()) do
        if v:IsA("RemoteEvent") and v.Name:lower():find("protection") then
            v:FireServer(true)
            if getgenv().notify then getgenv().notify("Success", "Bring Protection has been enabled successfully.", 5) end
        end
    end
end
