...
if getgenv().ParkourObbyMenuLoaded then return end
...
g.get_jobid = function() return game.JobId end
g.get_placeid = function() return game.PlaceId end
...
local function init_services()
    ...
    for _, serviceName in pairs(services) do g[serviceName] = cloneref and cloneref(game:GetService(serviceName)) or game:GetService(serviceName) end
end

init_services()
local lib = loadstring(game:HttpGet("https://gitlab.com/greatest-group/experience_coding/-/raw/main/UIs/Turtle.lua?ref_type=heads"))()
...
local function get_stages_folder_workspace()
    ...
    if stages_folder and stages_folder:IsA("Folder") then return stages_folder end
end
