...
local function get_char()
    local players = getgenv().Players or cloneref and cloneref(game:GetService("Players")) or game:GetService("Players")
    local lplr = getgenv().LocalPlayer or players.LocalPlayer
    local find_character = getgenv().Character or lplr.Character or g.get_char(lplr, 5)
...
getgenv().mod_arcade_high_score = function()
    ...
    if not ok then
        return getgenv().notify("Error", "An error occurred while setting maximum Arcade high-score: "..tostring(response), 15)
    else
        return getgenv().notify("Success", "Successfully modified high-score to the highest amount possible.", 5)
    end
end
...
getgenv().flowrgb = getgenv().flowrgb or function(connname,speed,obj,toggle)
    local ts = getgenv().TweenService or cloneref and cloneref(game:GetService("TweenService")) or game:GetService("TweenService")
    ...
    if toggle==false then
        ...
        getgenv()[connname]=rs.RenderStepped:Connect(function(dt)
            ...
            if typeof(obj) == "Instance" and obj:IsA("BasePart") then
                ...
                pcall(function() ts:Create(obj,TweenInfo.new(0.10,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut),{BackgroundColor3=col}):Play() end)
            end
        end)
    end
end

getgenv().find_body_orientation_module_cache = function()
    local cache = getgenv().body_orientation_module_found
    ...
    for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
        if v:IsA("ModuleScript") and v.Name:lower():find("body") and v.Name:lower():find("orientation") then
            getgenv().body_orientation_module_found = v
            return v
        end
    end
...
wait(0.1)
if not getgenv().body_orientation_module_found then pcall(function() getgenv().find_body_orientation_module_cache() end) end

getgenv().toggle_body_orientation_system = function(state)
    ...
    local Body_Orientation_Module_Found = getgenv().body_orientation_module_found or getgenv().find_body_orientation_module_cache()
    ...
    if state == true then
        getgenv().head_movement_system_enabled = false
        fn_disable()
    else
        getgenv().head_movement_system_enabled = true
        fn_enable()
    end
end
...
getgenv().find_start_game_RF = function()
    ...
    if cached and cached:IsA("RemoteFunction") then return cached end
    for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
...
wait(0.2)
getgenv().Toggle_Body_Orientation_UI = Tab_2:CreateToggle({
Name = "Disable Head Movement (FE)",
CurrentValue = getgenv().head_movement_system_enabled or false,
Flag = "Toggle_Head_Movement_In_UI",
Callback = function(state)
    getgenv().toggle_body_orientation_system(state)
end,})
