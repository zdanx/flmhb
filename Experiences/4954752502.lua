...
function bypass_anticheat()
    if getgenv().ac_bypass_already_loaded then
        return getgenv().notify and getgenv().notify("error", "Anti-Cheat bypass already loaded.", 7)
    end
    ...
    for _, func in getgc(false) do
        if typeof(func) == "function" then
            ...
            if source and source:find("Exploit_Defense") then
                local name = debug.info(func, "n") or "anonymous"
                
                -- Skip the two critical functions to keep stability
                if name == "Character_Check" or name == "Kick_Player" then
                    continue
                end
                
                local upvals = getupvalues(func)
                if #upvals > 0 then
                    hookfunction(func, noop)
                    count += 1
                end
            end
        end
    ...
    print(`[AC Bypass] ✅ Stably hooked {count}/14 functions (12 safe ones)`)
    wait(0.1)
