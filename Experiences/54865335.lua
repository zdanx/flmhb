...
getgenv().vehicle_speed_boost_toggle = function(toggled)
    if toggled == true then
        ...
        if not current_vehicle or not current_vehicle.Parent then
            if getgenv().Vehicle_Speed_Boost_Toggle then 
                getgenv().Vehicle_Speed_Boost_Toggle:Set({CurrentValue = false}) 
            end
            return getgenv().notify("Error", "No vehicle found.", 5)
        end

        local DriveSeat = current_vehicle:FindFirstChild("VehicleSeat") or current_vehicle:FindFirstChildOfClass("VehicleSeat")
        ...
        for _, obj in ipairs(DriveSeat:GetChildren()) do
            if obj.Name == "FlamesSpeedBoost" or obj.Name == "FlamesSpeedGyro" then
                obj:Destroy()
            end
        end
        wait(0.25)
        ...
        getgenv().vehicle_speed_boost_max_forward = getgenv().vehicle_speed_boost_max_forward or 350
        getgenv().vehicle_speed_boost_max_backward = getgenv().vehicle_speed_boost_max_backward or 120
        getgenv().vehicle_speed_boost_acceleration = getgenv().vehicle_speed_boost_acceleration or 12
        getgenv().notify("Success", "NSX Speed Boost Enabled", 2)
        getgenv().FlamesLibrary.spawn("vehicle_speed_boost_loop", "spawn", function()
            ...
            while getgenv().vehicle_speed_boost_active do
                task.wait(0.025)
                if not current_vehicle or not current_vehicle.Parent or not DriveSeat.Parent then break end
                local Max_Forward = getgenv().vehicle_speed_boost_max_forward or 350
                local Max_Backward = getgenv().vehicle_speed_boost_max_backward or 120
                local Acceleration = getgenv().vehicle_speed_boost_acceleration or 12
                local Throttle = DriveSeat.Throttle or 0
                if Is_Mobile then
                    pcall(function()
                        local ControlModule = require(Player.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("ControlModule"))
                        Throttle = -ControlModule:GetMoveVector().Z
                    end)
                end

                if Throttle > 0.1 then
                    S_Hold_Start = nil
                    Current_Speed = math.min(Current_Speed + Acceleration, Max_Forward)
                ...
                elseif Throttle < -0.1 then
                    ...
                    if tick() - S_Hold_Start >= 0.4 then
                        Current_Speed = math.max(Current_Speed - Acceleration * 0.8, -Max_Backward)
                    else
                        Current_Speed = math.max(Current_Speed - Acceleration * 2.5, 0)
                    end
                    BV.Velocity = DriveSeat.CFrame.LookVector * Current_Speed
                else
                    S_Hold_Start = nil
    ...
    elseif toggled == false then
        ...
        getgenv().FlamesLibrary.disconnect("vehicle_speed_boost_loop")
        getgenv().notify("Success", "NSX Speed Boost Disabled", 1.5)
    else
        return 
    end
end
...
local function enable()
    ...
    lib.connect("speedtraps_watch", workspace["_Main"]["SpeedTraps"].ChildAdded:Connect(function(v) pcall(function() v:Destroy() end) end))
end
...
local function mod_weapon(gun)
    ...
    task.wait()
        if Gun_Config:FindFirstChild("FireMode") then Gun_Config.FireMode.Value = "Automatic" end
        ...
        if weapon:GetAttribute("Ammo") then weapon:SetAttribute("Ammo", 9e9) end
    end
end
...
function lock_vehicle_func()
    local veh = retrieve_vehicle()
    if not veh then return end
    local find_controls_script = veh:FindFirstChild("ControlsScript", true)
    if not find_controls_script then return getgenv().notify("Error", "Could not find ControlsScript!", 5) end
    local re_controls_script = veh:FindFirstChild("ControlsScript", true):FindFirstChildOfClass("RemoteEvent")
    if not re_controls_script then return getgenv().notify("Error", "Could not find 'RemoteEvent' in ControlsScript!", 5) end
    if re_controls_script and re_controls_script:IsA("RemoteEvent") and re_controls_script.Parent then
        local args = {
            "Lock"
        }

        re_controls_script:FireServer(unpack(args))
    end
end

local function is_vehicle_locked()
    local veh = retrieve_vehicle()
    if not veh then return end
    local info_folder_veh = veh:FindFirstChild("Information", true)
    if not info_folder_veh then return getgenv().notify("Error", "Could not find Information Folder!", 5) end
    local states_folder_veh = info_folder_veh:FindFirstChild("States")
    if not states_folder_veh then return getgenv().notify("Error", "Could not find 'States' Folder inside Information Folder!", 5) end
    if states_folder_veh and states_folder_veh.Parent and states_folder_veh:GetAttribute("Locked") ~= nil then
        return states_folder_veh:GetAttribute("Locked")
    end
...
local last_check = 0
lib.connect("auto_lock_vehicle", RunService.Heartbeat:Connect(function()
    if not getgenv().auto_lock_vehicle_toggled_state then return end

    local now = tick()
    if now - last_check < 0.35 then return end
    last_check = now

    local locked = is_vehicle_locked()
...
getgenv().Auto_Lock_Vehicle_FE = Tab4:CreateToggle({
Name = "Auto Lock Vehicle (FE)",
CurrentValue = false,
Flag = "AutomaticallyLockCarFEToggle",
Callback = function(auto_lock_car)
    getgenv().auto_lock_vehicle_toggled_state = auto_lock_car
end,})
...
game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("Map"):WaitForChild("MapFunction"):InvokeServer(unpack(args))

game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("Data"):WaitForChild("ResetBounty"):InvokeServer()
--]]
...
getgenv().Vehicle_Speed_Boost_Config = getgenv().Vehicle_Speed_Boost_Config or {
    Enabled = false,
    Forward = 425,
    Backward = 125,
    Accel = 10
...
getgenv().Vehicle_Speed_Boost_Toggle = Tab1:CreateToggle({
Name = "Vehicle Speed Boost",
CurrentValue = getgenv().vehicle_speed_boost_active,
Flag = "VB_Main",
Callback = function(toggle)
    getgenv().vehicle_speed_boost_toggle(toggle)
end,})
...
Range = {50, 1000},
Increment = 5,
CurrentValue = getgenv().vehicle_speed_boost_max_forward or 150,
Flag = "VB_Forward",
Callback = function(v)
    getgenv().vehicle_speed_boost_max_forward = v
end,})
...
Range = {25, 500},
Increment = 5,
CurrentValue = getgenv().vehicle_speed_boost_max_backward or 50,
Flag = "VB_Backward",
Callback = function(v)
    getgenv().vehicle_speed_boost_max_backward = v
end,})
...
Range = {1, 50},
Increment = 1,
CurrentValue = getgenv().vehicle_speed_boost_acceleration or 5,
Flag = "VB_Accel",
Callback = function(v)
    getgenv().vehicle_speed_boost_acceleration = v
end,})
...
Callback = function()
    ...
    loadstring(game:HttpGet("https://pastebin.com/raw/JYs67Yfm"))()
end,})